...
const defaultI18nInterpolation = { prefix: '{{', suffix: '}}' };
const safeI18nInterpolation = { interpolation: { prefix: '[[', suffix: ']]' } };

function shouldBypassI18nextInterpolation() {
  const interpolation = (i18n.options && i18n.options.interpolation) || {};
  const prefix = interpolation.prefix || defaultI18nInterpolation.prefix;
  const suffix = interpolation.suffix || defaultI18nInterpolation.suffix;
  return prefix === defaultI18nInterpolation.prefix &&
    suffix === defaultI18nInterpolation.suffix;
}
...
function translateElem($, elem, options, t) {
  ...
  // Keep {{ }} placeholders intact for static-i18n when i18next uses defaults.
  const shouldBypassInterpolation = interpolate && shouldBypassI18nextInterpolation();
  let trans = shouldBypassInterpolation ? t(key, safeI18nInterpolation) : t(key);
  if (interpolate) {
